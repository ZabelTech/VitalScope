...
- **Protocols**: two tables. `protocols(id PK, name, category IN ('drug','peptide','ped','supplement_stack','hormesis','fasting','training_block'), dose, unit, cadence, start_date, end_date, notes, created_at)` — one row per intervention protocol. `protocol_events(id PK, protocol_id FK, date, time, dose, duration_minutes, notes, created_at)` — actual logged occurrences, `ON DELETE CASCADE` from protocols. Indexes on `date` and `protocol_id`. For hormesis quick-logs (Zone 2, Sauna, Cold Plunge) the backend's `find-or-create` logic lives in the **frontend** (`findOrCreateQuick` in `ProtocolsSection.tsx`), not the backend — keep it there.
