...
- **Blood pressure**: `blood_pressure_entries(id PK, date, time, systolic_mmhg, diastolic_mmhg, pulse_bpm, notes, created_at)` — manual entry only (no sync source). `GET/POST /api/blood-pressure`, `DELETE /api/blood-pressure/{id}`. POST validates `systolic > diastolic` and returns 400 otherwise. Multiple readings per date are allowed (no upsert). Frontend: `BloodPressureSection.tsx` wired into `OrientPage` as the "Blood pressure" section (separate from `LongevitySection`); the latest reading also surfaces as an `AgeBadge` card in `TodayMetrics.tsx` (Observe → Today's metrics). AI context category `blood_pressure` (Health group) is registered but no prompt-builder consumes it yet — wire it through `_gather_*` helpers when an analysis depends on BP.
